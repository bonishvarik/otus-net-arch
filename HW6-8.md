...
Следующим шагом по настройке будет конфигурирование EVPN ESI Multihoming в режиме All-Active. Данный функционал позволит подключить клиентское оборудование к двум разным Leaf, что обеспечивает отказоустойчивость в случае выхода из строя одного из Leaf или линка до него. Двумя линками к нам будет подключаться Client-A-2, конфигурация на нем будет выглядеть следующим образом: 
<details>
...
Маршруты присутствую. А как выглядит маршрут с MAC-адресом Client-A-2 на J-Leaf-1? Отфильтруем вывод по MAC-адресу. Но для начала нужно узнать этот MAC: 
```
...
2:10.255.0.22:1::10010::aa:bb:cc:80:0d:00/304               
                   *[BGP/170] 00:05:40, localpref 100, from 10.255.0.1
                    ...
                    > to 10.255.0.2 via xe-0/0/9.0
```
Как видно, это все тот же маршрут второго типа, поскольку именно он используется для распространения информации о MAC-адресах. На данном этапе настройку ESI можно считать завершенной. 
